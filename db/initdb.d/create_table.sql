...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='대량 이용권, 다수의 이용자에게 이용권 지급 목적';
...
(
    `user_group_id`     varchar(20) NOT NULL COMMNET '사용자 그룹 ID',
    `user_id`           varchar(20) NOT NULL COMMENT '사용자 ID',
...
(
    `notification_seq`  int             NOT NULL AUTO_INCREMENT COMMENT '알림 순번',
    `uuid`              varchar(20)     NOT NULL COMMENT '사용자 uuid(KAKAO)',
    `event`             varchar(10)     NOT NULL COMMENT '이벤트',
    `text`              varchar(1000)   NOT NULL COMMENT '알림 내용',
